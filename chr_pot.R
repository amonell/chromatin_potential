# chromatin potential -----------------------------------------------------
SEsum_smooth <- as.matrix(read.table('smoothed_dorc_table_my_umap.csv', sep = ',', header = TRUE))
forcols <- read.table('smoothed_dorc_columns.csv', sep = ',')
temp_cell_index <- read.table('foxa1_data/G2peakcelltypes.txt', sep = "\t", header = TRUE)
...
RNA_smooth <- as.matrix(read.table('foxa1_data/smoothed_rna_table_G2_40.csv', sep = ',', header = TRUE))
...
celltype <- read.table('foxa1_data/G2luminalvsbasal.tsv', sep = '\t')
celltype <- celltype$x
...
genes <- names(y[y > 0.029]) 
length(genes) # try to get top half of genes
last = genes

library(readr)
mapcoords <- read.table('foxa1_data/archerumapsg2.tsv', sep = '\t', header = TRUE)
...
plot(umap.smooth$umap1, umap.smooth$umap2, col=umap.smooth$Group)
...
ggplot(umap.smooth, aes(x=umap1, y=umap2)) +
  ...
  pretty_plot(fontsize = 5) + L_border() + xlim(-11, -2) + ylim(-7, 2)
  guides(colour = FALSE)
